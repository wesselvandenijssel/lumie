...
These instructions define the expected standards for all Claude Code completions in this repository. Claude should prefer the examples, syntax, and file structures shown below.
...
Each Local site lives under `~/Local Sites/<site-name>/` and contains a theme with the **same name** as the site. The theme name varies per project — `base-theme` is just the template repo this structure originated from.
...
The npm scripts (see `package.json`) set `NODE_ENV` via `cross-env`:

- `npm run dev` → `NODE_ENV=dev`
- `npm run dev-watch` → `NODE_ENV=dev-watch` (BrowserSync watch mode)
...
- The font families live in `_config.scss`: `$fa` (`"Font Awesome 6 Pro"`) for solid/regular/light, `$fab` (`"Font Awesome 6 Brands"`) for brand icons.
...
**Why:** class-names change between FA major versions (`fa` → `fas` → `fa-solid`), which would break markup across many templates; unicode codepoints are stable and live in one file. It also avoids shipping/relying on the full FA kit CSS for class resolution.
...
if (!empty($buttons) && is_array($buttons)) {
    $button_group = new BlockButtons($buttons);
    echo $button_group->getButtons();
}
```
