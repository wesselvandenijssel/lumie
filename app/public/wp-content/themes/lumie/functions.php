...
function lumie_setup() {
	// let's get language support going, if you need it
	load_theme_textdomain('lumie', get_template_directory() . '/src/languages');

	// launching operation cleanup
	add_action('init', 'lumie_head_cleanup');
	// A better title
	add_filter('wp_title', 'rw_title', 10, 3);
	// remove WP version from RSS
	add_filter('the_generator', 'lumie_rss_version');
	// remove pesky injected css for recent comments widget
	add_filter('wp_head', 'lumie_remove_wp_widget_recent_comments_style', 1);
	// clean up comment styles in the head
	add_action('wp_head', 'lumie_remove_recent_comments_style', 1);

	// enqueue base scripts and styles
	add_action('wp_enqueue_scripts', 'lumie_scripts_and_styles', 999);

	// cleaning up random code around images
	add_filter('the_content', 'lumie_filter_ptags_on_images');
	// cleaning up excerpt
	add_filter('excerpt_more', 'lumie_excerpt_more');
}

// let's get this party started
add_action('after_setup_theme', 'lumie_setup');
